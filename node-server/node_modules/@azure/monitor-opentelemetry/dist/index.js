...
class Logger {
    ...
    constructor() {
        ...
        if (azureLogLevel) {
            logger.setLogLevel(azureLogLevel);
        }
...
(function (QuickPulseOpenTelemetryMetricNames) {
    ...
    QuickPulseOpenTelemetryMetricNames["PROCESSOR_TIME"] = "azureMonitor.processorTotalProcessorTime";
...
function resourceMetricsToQuickpulseDataPoint(metrics, baseMonitoringDataPoint, documents) {
    let metricPoints = [];
    ...
    let quickpulseDataPoint = Object.assign(Object.assign({}, baseMonitoringDataPoint), { timestamp: new Date(), metrics: metricPoints, documents: documents });
...
function getSpanDocument(span) {
    ...
    if (span.kind === api.SpanKind.SERVER || span.kind === api.SpanKind.CONSUMER) {
        ...
        document = {
            ...
            url: url,
            responseCode: code,
            duration: getIso8601Duration(core.hrTimeToMilliseconds(span.duration)),
        };
    }
    else {
        url = getUrl(span.attributes);
...
function getLogDocument(logRecord) {
    let document = {
        documentType: KnownDocumentType.Exception,
    };
    const exceptionType = String(logRecord.attributes[semanticConventions.SEMATTRS_EXCEPTION_TYPE]);
    if (exceptionType) {
        const exceptionMessage = String(logRecord.attributes[semanticConventions.SEMATTRS_EXCEPTION_MESSAGE]);
        document = {
            documentType: KnownDocumentType.Exception,
...
class LiveMetrics {
    ...
    recordSpan(span) {
        if (this.isCollectingData) {
            ...
            let document = getSpanDocument(span);
            this.addDocument(document);
            const durationMs = core.hrTimeToMilliseconds(span.duration);
            let success = span.status.code !== api.SpanStatusCode.ERROR;
            if (span.kind === api.SpanKind.SERVER || span.kind === api.SpanKind.CONSUMER) {
    ...
    recordLog(logRecord) {
        if (this.isCollectingData) {
            let document = getLogDocument(logRecord);
            this.addDocument(document);
            if (isExceptionTelemetry(logRecord)) {
                this.totalExceptionCount++;
            }
    ...
    getCommitedMemory(observableResult) {
        var freeMem = os__namespace.freemem();
        var committedMemory = os__namespace.totalmem() - freeMem;
...
class LogHandler {
    ...
    _initializeInstrumentations() {
        ...
        if ((_a = this._config.instrumentationOptions.bunyan) === null || _a === void 0 ? void 0 : _a.enabled) {
            this._instrumentations.push(new instrumentationBunyan.BunyanInstrumentation(this._config.instrumentationOptions.bunyan));
        }
    }
}
...
class BrowserSdkLoader {
    ...
    _getBrowserSdkLoaderReplacedStr() {
        let osStr = getOsPrefix();
        let rpStr = getResourceProvider();
        let sdkLoaderReplacedStr = `${this._browserSdkLoaderIkey}\",\r\n disableIkeyDeprecationMessage: true,\r\n sdkExtension: \"${rpStr}${osStr}d_n_`;
        let replacedSdkLoader = applicationinsightsWebSnippet.webSnippet.replace("INSTRUMENTATION_KEY", sdkLoaderReplacedStr);
            ...
            if (originalRequestListener) {
                requestListener = (request, response) => {
                    ...
                    response.write = function wrap(a, b) {
                        ...
                        try {
                            if (isGetRequest) {
                                let headers = getContentEncodingFromHeaders(response);
                    ...
                    res.write = function wrap(a, b) {
                        ...
                        try {
                            if (isGetHttpsRequest) {
                                let headers = getContentEncodingFromHeaders(res);
                                let writeBufferType = undefined;
                                if (typeof b === "string") {
                                    writeBufferType = b;
                                }
    ...
    ValidateInjection(response, input) {
        try {
            if (!response || !input || response.statusCode != 200)
                return false;
            let isContentHtml = isContentTypeHeaderHtml(response);
            if (!isContentHtml)
                return false;
     ...
     */
    InjectSdkLoader(response, input, encodeType, bufferEncodeType) {
        try {
            ...
            if (!isCompressedBuffer) {
                ...
                if (index < 0)
                    return input;
                let newHtml = insertBrowserSdkLoaderByIndex(index, html, BrowserSdkLoader._sdkLoader);
                if (typeof input === "string") {
                    ...
                    let isValidBufferType = isBufferType(input, bufferType);
        ...
        try {
            switch (encodeType) {
                case contentEncodingMethod.GZIP:
                    let gunzipBuffer = zlib__namespace.gunzipSync(input);
                ...
                case contentEncodingMethod.DEFLATE:
                    let inflateBuffer = zlib__namespace.inflateSync(input);
                ...
                case contentEncodingMethod.BR:
                    let BrotliDecompressSync = getBrotliDecompressSync(zlib__namespace);
                    let BrotliCompressSync = getBrotliCompressSync(zlib__namespace);
...
function useAzureMonitor(options) {
    ...
    let spanProcessors = (options === null || options === void 0 ? void 0 : options.spanProcessors) || [];
    // Add batch processor as the last one
    spanProcessors.push(traceHandler.getBatchSpanProcessor());
    // Add extra SpanProcessors, MetricReaders and LogRecordProcessors
    let logRecordProcessors = (options === null || options === void 0 ? void 0 : options.logRecordProcessors) || [];
    // Add batch processor as the last one
    logRecordProcessors.push(logHandler.getBatchLogRecordProcessor());
    try {
        const tracerProvider = api.trace.getTracerProvider().getDelegate();
        spanProcessors.forEach((spanProcessor) => {
            tracerProvider.addSpanProcessor(spanProcessor);
        });
    }
    catch (error) {
        Logger.getInstance().error("Failed to add SpanProcessors to TracerProvider.", error);
    }
    try {
        ...
        Logger.getInstance().error("Failed to add LogRecordProcessors to LoggerProvider.", error);
    }
}
/**
