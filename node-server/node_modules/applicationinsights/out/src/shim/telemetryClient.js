...
class TelemetryClient {
    ...
    trackDependency(telemetry) {
        ...
        if (telemetry.dependencyTypeName) {
            if (telemetry.dependencyTypeName.toLowerCase().indexOf("http") > -1) {
                ...
                attributes[semantic_conventions_1.SemanticAttributes.RPC_SYSTEM] = telemetry.dependencyTypeName;
                attributes[semantic_conventions_1.SemanticAttributes.RPC_METHOD] = telemetry.name;
                attributes[semantic_conventions_1.SemanticAttributes.RPC_GRPC_STATUS_CODE] = telemetry.resultCode;
            }
        }
        if (telemetry.target) {
    ...
    getAuthorizationHandler(config) {
        api_1.diag.warn("getAuthorizationHandler is not supported in ApplicationInsights any longer.");
    }
    /*
    ...
    addTelemetryProcessor(telemetryProcessor) {
        api_1.diag.warn("addTelemetryProcessor is not supported in ApplicationInsights any longer.");
    }
    /*
