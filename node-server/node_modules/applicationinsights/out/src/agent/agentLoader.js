...
class AgentLoader {
    ...
    _sdkAlreadyExists() {
        try {
            ...
            try {
                ...
                if (this._isWindows) {
                    ...
                    distroInstance = require.resolve("@azure/monitor-opentelemetry", { paths: [process.cwd()] });
                }
                // Node 8.9+ Linux
                else if (this._isLinux) {
                    shimInstance = `${process.cwd()}${require.resolve("applicationinsights", { paths: [process.cwd()] })}`;
                    distroInstance = `${process.cwd()}${require.resolve("@azure/monitor-opentelemetry", { paths: [process.cwd()] })}`;
                }
            }
            catch (e) {
                // Node <8.9
        ...
        */
            if (shimInstance.indexOf("home") > -1 || distroInstance.indexOf("home") > -1 ||
